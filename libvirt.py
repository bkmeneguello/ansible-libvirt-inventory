...
# TODO: Read metadata from https://libvirt.org/formatdomain.html#elementsMetadata


def dict2xml(values, name):
    root = lxml.etree.Element(name)
    return lxml.etree.tostring(buildxml(root, values))


def buildxml(root, values):  # type: (lxml.etree.Element, Union[Dict, str, Any]) -> lxml.etree.Element
    if isinstance(values, dict):
        for k, v in values.items():
            if isinstance(v, (tuple, list)):
                for item in v:
                    s = lxml.etree.SubElement(root, k)
                    buildxml(s, item)
            else:
                s = lxml.etree.SubElement(root, k)
                buildxml(s, v)
...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def parse(self, inventory, loader, path, cache=False):
        ...
        for vir_dom in conn.listAllDomains():
            ...
            if vir_dom.isActive():
                ...
                root = lxml.etree.fromstring(dict2xml(addrs, 'addrs'))  # type: lxml.etree.Element
                addr = root.find(self.get_option('host_xpath'))  # type: lxml.etree.Element
                if addr is not None:
    ...
    def set_groups(self, vir_dom, host):  # type: (libvirt.virDomain, str) -> None
        root = lxml.etree.fromstring(vir_dom.XMLDesc())
